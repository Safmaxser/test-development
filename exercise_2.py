...
def number_of_names(mentors):
    ...
    for name in unique_names:
        code = calc_name_code(name)
        if dict_names_codes.get(code) is None:
            dict_names_codes[code] = {name}
        else:
            dict_names_codes[code].add(name)
    return dict_names_codes
...
if __name__ == '__main__':
    mentors_m = [
        ...
        ["Филипп Воронов", "Анна Юшина", "Иван Бочаров", "Анатолий Корсаков",
         ...
         "Никита Шумский", "Алексей Степанов", "Денис Коротков",
         "Антон Глушков",
    ...
    for number in range(1, 10):
        result = number_of_names(mentors_m).get(number)
        print(codes_info[number])
        if result is not None:
            print(f"Коду {number} соответствуют: {', '.join(sorted(result))}")